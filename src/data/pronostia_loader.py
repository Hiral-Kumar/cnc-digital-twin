...
class PronostiaLoader:
    ...
    def load_bearing(self,
        ...
        Returns:
            dict:
                ...
                'n_bursts'    : int
        """
        bearing_dir = self._resolve_bearing_dir(bearing_subdir)

        if bearing_dir is None:
            expected_dir = os.path.join(self.root_dir, bearing_subdir)
            raise FileNotFoundError(
                f"Bearing directory not found: {expected_dir}\n"
                f"Update config/config.yaml → data.pronostia.root_dir\n"
                f"If you extracted the official archive, check whether the files are under "
                f"Training_set/Learning_set/{os.path.basename(bearing_subdir)} instead."
            )
    ...
    def _resolve_bearing_dir(self, bearing_subdir: str) -> Optional[str]:
        """Resolve the PRONOSTIA bearing directory across common unzip layouts."""
        candidates = [os.path.join(self.root_dir, bearing_subdir)]

        parts = bearing_subdir.replace('\\', '/').split('/')
        if len(parts) >= 2 and parts[0] == 'Training_set':
            alternate = os.path.join(self.root_dir, 'Training_set', 'Learning_set', *parts[1:])
