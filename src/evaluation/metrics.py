...
def phm2012_score(y_true: np.ndarray, y_pred: np.ndarray,
    ...
    late_arg = -np.log(0.5) * (Er / late_denom)
    early_arg = np.log(0.5) * (Er / early_denom)
    A = np.where(
        Er <= 0,
        np.exp(np.clip(late_arg, -700.0, 700.0)),   # late: steeper decay
        np.exp(np.clip(early_arg, -700.0, 700.0))    # early: gentler decay
    )
...
def pearson_rho(predicted_rates: np.ndarray,
    ...
    finite_mask = np.isfinite(predicted_rates) & np.isfinite(reference_rates)
    predicted_rates = predicted_rates[finite_mask]
    reference_rates = reference_rates[finite_mask]

    if len(predicted_rates) < 2:
        logger.warning("Pearson ρ requires at least 2 finite samples. Returning 0.0")
        return 0.0
