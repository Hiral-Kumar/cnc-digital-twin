...
class ODEFunction(nn.Module):
    ...
    def forward(self, t: torch.Tensor, h: torch.Tensor) -> torch.Tensor:
        ...
        # Keep the solver numerically safe when the state or time drifts
        # outside the finite range during inference or early training.
        ...
        dh_dt = torch.nan_to_num(dh_dt, nan=0.0, posinf=1e3, neginf=-1e3)
        dh_dt = torch.clamp(dh_dt, -10.0, 10.0)
...
class NeuralODE(nn.Module):
        ...
        if self.use_adjoint and self.training:
            ...
            with torch.no_grad():
                try:
                    ...
                    logger.warning(
                        f"Adaptive solver failed during inference ({exc}); "
                        "falling back to a manual Euler rollout."
                    )
                    trajectory = self._euler_rollout(h0, t_span)

                if not torch.isfinite(trajectory).all():
                    logger.warning(
                        "Inference integration produced non-finite values; "
                        "using manual Euler rollout instead."
                    )
                    trajectory = self._euler_rollout(h0, t_span)
    ...
    def _euler_rollout(self, h0: torch.Tensor, t_span: torch.Tensor) -> torch.Tensor:
        """Deterministic finite-step rollout used when adaptive integration is unstable."""
        trajectory = [h0]
        h = h0
        if len(t_span) < 2:
            return torch.stack(trajectory, dim=0)

        dt = (t_span[-1] - t_span[0]) / float(len(t_span) - 1)
        t = t_span[0]

        for _ in range(1, len(t_span)):
            dh_dt = self.ode_func(t, h)
            h = h + dt * dh_dt
            h = torch.nan_to_num(h, nan=0.0, posinf=1e3, neginf=-1e3)
            trajectory.append(h)
            t = t + dt
